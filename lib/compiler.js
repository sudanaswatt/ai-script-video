...
`
  ...
  const imagePrompt = `
Buat gambar iklan portrait sinematik rasio 9:16 (vertikal).
...
Karakter (HARUS sama di semua video berikutnya):
${dna}

Karakter sedang berinteraksi secara natural dengan produk:
"${input.product}"

Lokasi dan lingkungan (TERKUNCI):
${input.objectVisual}

Lingkungan harus tetap sama persis di semua video nanti.
Jangan mengubah struktur ruangan, tata letak, arah cahaya, atau latar belakang.
...
Tidak boleh latar fantasi atau artificial.
`.trim();
...
- Hanya ambience ruangan yang halus.

Gerakan mikro:
- Kedipan mata natural.
- Napas halus.
- Perpindahan berat badan sangat ringan.
